...
//spring security 에서 기본으로 제공하는 OAuth2UserService 을 상속받아 커스텀
@Service
public class OAuth2PrincipalUserService extends DefaultOAuth2UserService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        // Spring Security 가 OAuth2 provider 에게 AccessToken 으로 사용자 정보를 요청
        ...
        // 어떤 OAuth2 Provider 인지 확인
        // provider => 공급처 (google, naver, kakao)
        String provider = userRequest.getClientRegistration().getRegistrationId();

        // 로그인 한 사용자의 식별자(id), 이메일
        // 로그인 시 사용한 이메일
        String email = null;
        // 공급처에서 발행한 사용자 식별자
        String id = null;

        // provider 종류에 따라서 사용자 정보 패싱 방식이 다르므로 분기 처리
        switch (provider) {
            case "google":
